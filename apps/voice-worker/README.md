...
- **openai**: OpenAI STT/TTS/LLM (`OPENAI_API_KEY`, optional `SWARAI_OPENAI_LLM_MODEL`).
...
export SWARAI_APP_URL=http://localhost:3000   # Next.js URL reachable from this process
export SWARAI_INTERNAL_WEBHOOK_SECRET=...    # must match web app
...
See root `docker-compose.yml` — the worker service installs this package and runs `python -m swarai_agent start` (production-style).
